...
const defaultKeywords = ["產品經理", "PM", "AI PM", "Product Manager"];
const defaultExcludeCompanies: string[] = [];
...
export function Jobs() {
  ...
  const [excludeCompanies, setExcludeCompanies] = useState<string[]>(defaultExcludeCompanies);
  const [newKeyword, setNewKeyword] = useState("");
  const [newCompany, setNewCompany] = useState("");
  const [selectedCities, setSelectedCities] = useState<string[]>(["taipei", "newtaipei"]);
  ...
  const addCompany = () => {
    ...
    if (c && !excludeCompanies.includes(c)) setExcludeCompanies([...excludeCompanies, c]);
    setNewCompany("");
  };
  ...
  const toggleCity = (val: string) => {
    ...
    const next = selectedCities.filter(c => c !== "all");
    if (next.includes(val)) {
      setSelectedCities(next.filter(c => c !== val));
    } else {
      setSelectedCities([...next, val]);
    }
  };
    ...
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          設定 104 職缺搜尋條件，每天下午 5 點推播新增與變動職缺
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-border shadow-sm">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button variant="outline" className="rounded-xl" onClick={addKeyword}>
          ...
          <Card className="border-border shadow-sm">
            <CardHeader>
              ...
              <CardDescription>可複選，選「全台」會覆蓋其他設定</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2">
          ...
          <Card className="border-border shadow-sm">
            ...
            <CardContent className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {excludeCompanies.map((c) => (
                    ...
                    >
                      {c}
                      <button
                        onClick={() => setExcludeCompanies(excludeCompanies.filter(x => x !== c))}
              ...
              <div className="flex gap-2">
                <Input
                  placeholder="公司名稱，Enter 確認"
                  value={newCompany}
                  onChange={(e) => setNewCompany(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && addCompany()}
                  className="rounded-xl border-border bg-white"
                />
                <Button variant="outline" className="rounded-xl" onClick={addCompany}>
                  <Plus className="w-4 h-4" />
                </Button>
              </div>
            </CardContent>
              ...
              <div className="space-y-2">
                ...
                <Select defaultValue="40">
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="40">40K 以上</SelectItem>
                    <SelectItem value="50">50K 以上</SelectItem>
                    <SelectItem value="60">60K 以上</SelectItem>
                    <SelectItem value="80">80K 以上</SelectItem>
                  </SelectContent>
                </Select>
              </div>
          ...
          <Card className="border-border shadow-sm">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border">
                <div>
                  <p className="text-sm font-medium text-foreground">只通知新職缺</p>
                  <p className="text-xs text-muted-foreground">過濾重複出現的職缺</p>
                </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border">
                <div>
                  <p className="text-sm font-medium text-foreground">薪資變動通知</p>
                  <p className="text-xs text-muted-foreground">職缺薪資調整時推播</p>
                </div>
                <Switch defaultChecked={true} />
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border">
                <div>
                  <p className="text-sm font-medium text-foreground">下架職缺通知</p>
                  <p className="text-xs text-muted-foreground">追蹤的職缺下架時推播</p>
                </div>
